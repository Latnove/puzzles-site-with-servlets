...
<head>
    ...
    <link rel="stylesheet" href="${contextPath}/styles/play.css" />

    <script>
        const IMAGE_URL = "${puzzle.imageUrl}";
        const COLS = ${puzzle.pieceCols};
        const ROWS = ${puzzle.pieceRows};
        const PUZZLE_ID = ${puzzle.id};
    </script>

    <script defer src="${contextPath}/scripts/puzzlesUtils.js"></script>
    <script defer src="${contextPath}/scripts/play.js"></script>
</head>
<body>
<a href="${contextPath}/puzzle/${puzzle.id!}" class="button button-secondary back">
    ...
    <span>Вернуться к пазлу</span>
</a>
<main class="play">
    <div class="container play__container">
        <h1 class="title play__title">Вы начали собирать пазл "${puzzle.title?html}"</h1>
        <div class="puzzle-container"></div>

        <div class="all-puzzles"></div>
    </div>
    ...
    <div class="modal-overlay">
        <div class="modal">
            ...
            <form method="post">
                <button class="button modal-button">Продолжить</button>
            </form>
        </div>
    </div>
</main>
